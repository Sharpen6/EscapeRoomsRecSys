...
class RunPrecisionRecall():
    ...
    def test_top_n_algorithms(self):
        ...
        train_set = pd.read_csv(train_set_path, index_col='index')
        test_set = pd.read_csv(test_set_path, index_col='index')

        train_set['timestamp'] = pd.to_datetime(train_set['timestamp'], format="%d/%m/%Y")
        test_set['timestamp'] = pd.to_datetime(test_set['timestamp'], format="%d/%m/%Y")
    ...
    def calc_precision_auc(self, train_set, test_set, algs_results):
        ...
        for method, (recommendations, filled_with_pop, clean_faked) in algs_results.items():
            ...
            df_stats = df_stats.append(
                {'alg': method, 'map': MAP, 'prec@5': Prec_5, 'prec@10': Prec_10, 'recall@5': Recall_5,
                 'recall@10': Recall_10}, ignore_index=True)
        return df_stats
...
if __name__=='__main__':
    rpr = RunPrecisionRecall()
    rpr.test_top_n_algorithms()